...
def detect(save_img=False):
    ...
    #Load model 2
    model1 = attempt_load('gunNduckAugmentFinal.pt', map_location=device)  # load FP32 model
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred1):  # detections per image
            ...
            if det is not None and len(det):
                ...
                # Write results
                for *xyxy, conf, cls in reversed(det):
                    toReturn += [names1[int(cls)],(xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist(), '%.2f' % (conf)]
                    if save_txt: # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywhs
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * 5 + '\n') % (cls, *xywh))  # label format
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im0, label=label, color=colors1[int(cls)], line_thickness=3)          
